...
%% Start before conn handshake, with only Conn handle
-export([
    start_link/3,
    start_link/4,
    start/4,
...
start_link(Callback, Stream, Conn, StreamOpts, Props) ->
    start_link(Callback, Stream, Conn, StreamOpts, Props, []).
start_link(Callback, Stream, Conn, StreamOpts, Props, GenStartOpts) when
    Callback =/= undefined andalso
        is_atom(Callback) andalso
        is_map(Props)
->
    gen_server:start_link(
        ?MODULE, [Callback, Stream, Conn, StreamOpts, Props, self()], GenStartOpts
    ).

start(Callback, Stream, Conn, StreamOpts, Props, GenStartOpts) when
    Callback =/= undefined andalso
        is_atom(Callback) andalso
        is_map(Props)
->
...
init([Callback, Stream, Conn, StreamOpts, Props, PrevOwner]) ->
    ...
    case Callback:init_handoff(Stream, StreamOpts, Conn, Props) of
        {ok, CBState} ->
            %% not yet takeover the ownership
            State = #{
                is_owner => false,
                is_local => false,
                stream_opts => StreamOpts,
...
->
    ...
    ?tp_ignore_side_effects_in_prod(stream_data, #{
        module => ?MODULE, stream => Stream, buffer => Buffer
    }),
...
handle_info(
    {quic, start_completed, Stream,
        #{
            status := _AtomStatus,
            stream_id := _StreamId,
            is_peer_accepted := _PeerAccepted
        } = Props},
    #{
        callback := M,
        callback_state := CBState
    ...
    ?tp_ignore_side_effects_in_prod(debug, #{
        module => ?MODULE, event => start_completed, props => Props
    }),
    default_cb_ret(M:start_completed(Stream, Props, CBState), State);
handle_info(
    {quic, send_complete, Stream, IsSendCanceled},
    #{
        callback := M,
        callback_state := CBState
    } = State
    ...
    ?tp_ignore_side_effects_in_prod(debug, #{
        module => ?MODULE, event => send_complete, is_canceled => IsSendCanceled
    }),
...
handle_info(
    {quic, peer_send_aborted, Stream, ErrorCode},
    #{
        callback := M,
        callback_state := CBState
    } = State
    ...
    ?tp_ignore_side_effects_in_prod(debug, #{
        module => ?MODULE, event => peer_send_aborted, error_code => ErrorCode
    }),
    default_cb_ret(M:peer_send_aborted(Stream, ErrorCode, CBState), State);
handle_info(
    {quic, peer_receive_aborted, Stream, ErrorCode},
    #{
        callback := M,
        callback_state := CBState
    } = State
    ...
    ?tp_ignore_side_effects_in_prod(debug, #{
        module => ?MODULE, event => peer_receive_aborted, error_code => ErrorCode
    }),
    default_cb_ret(M:peer_receive_aborted(Stream, ErrorCode, CBState), State);
handle_info(
    {quic, send_shutdown_complete, Stream, IsGraceful},
    #{
        callback := M,
        callback_state := CBState
    } = State
    ...
    ?tp_ignore_side_effects_in_prod(debug, #{
        module => ?MODULE, event => send_shutdown_complete, is_graceful => IsGraceful
    }),
...
) when C =/= undefined andalso is_map(Flags) ->
    ?tp_ignore_side_effects_in_prod(debug, #{
        module => ?MODULE, conn => C, stream => Stream, event => stream_closed, flags => Flags
    }),
    default_cb_ret(M:stream_closed(Stream, Flags, CbState), State);
handle_info(
    {quic, peer_accepted, Stream, undefined},
    #{
        callback := M,
        callback_state := CBState
    } = State
